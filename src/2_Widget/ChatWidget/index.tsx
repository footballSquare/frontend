...
import useIsExpanded from "./model/useIsExpanded";
...
const ChatWidget = (props: ChatWidgetProps) => {
  ...
  const { isExpanded, toggleExpanded } = useIsExpanded({
    isFloating,
  });
  ...
  if (isPage && !isFloating) {
    return (
      <div className="bg-gray-900 border border-gray-700/50 w-full h-[80vh] rounded-lg shadow-lg flex flex-col overflow-hidden transition-all duration-200 relative">
        {/* 헤더 */}
  ...
  // ─── 채팅을 화면에 표시 모드 (isPage === false && isFloating === true) ───
  if (!isPage && isFloating) {
    return (
